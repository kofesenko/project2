...
# Production
resource "aws_security_group" "production_public" {
  name        = "production-public"
  description = "Security Group for public production"
...
resource "aws_security_group" "production_private" {
  name        = "production-private"
  description = "Security Group for private production"
  vpc_id      = module.vpc_production.vpc_id

  ingress {
    description = "from production-public sg"
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
    security_groups = [aws_security_group.production_public.id]
  }
